...
rule star_chip_mapping_library:
    ...
    output:
        ...
        chimsam=temp(os.path.join("STAR_chimeric","library","{samplename}.Chimeric.out.sam"))
    params:
...
rule chimeric2igv_junctions:
    ...
    output:
        junc=os.path.join("STAR_chimeric","library_IGV_junctions","{samplename}.Chimeric.out.junction.bed")
    params:
        span_cutoff = 150000,
        min_read_counts = 2,
        script = os.path.join(maindir, "rules", "scripts", "chimeric2igv_junction_bed.R")
    shell:
        "Rscript {params.script} {params.span_cutoff} {params.min_read_counts} {input.junc} > {output.junc}"


rule chimericSam2Bam:
    input:
        sam=os.path.join("STAR_chimeric","library","{samplename}.Chimeric.out.sam")
    output:
        bamtmp=temp(os.path.join("STAR_chimeric","library","{samplename}.Chimeric.out.tmp.bam")),
        bam=os.path.join("STAR_chimeric","library","{samplename}.Chimeric.out.bam"),
        bai=os.path.join("STAR_chimeric","library","{samplename}.Chimeric.out.bam.bai")
    threads: 8
    shell:
        """
        sambamba -q view -t {threads} -S -f bam -o {output.bamtmp} {input.sam} &&
        sambamba -q sort  -t {threads} -o {output.bam} {output.bamtmp}
        """

...
```{r eval=FALSE}
enrichment_set=paste0(list.files('../iCLIP.introns_analysis.elav_spliced/iCLIP_enrichment/',pattern='.*genes_.*.tsv$', full.names=TRUE),collapse=',')
centrality_set=paste0(list.files('../iCLIP.introns_analysis.elav_spliced/iCLIP_centrality/',pattern='.*genes_.*.tsv$', full.names=TRUE),collapse=',')
profile_set=paste0(list.files('../iCLIP.introns_analysis.elav_spliced/iCLIP_profile/',pattern='.*genes_.*.tsv$', full.names= TRUE),collapse=',')
```
...
enrich.set = enrich.set %>% mutate(group = gsub('(.*).genes_(.*)','\\1', L1),
                                   feature = gsub('(.*).genes_(.*)','\\2',L1))
centra.set = centra.set %>% mutate(group = gsub('(.*).genes_(.*)','\\1',L1),
                                   feature = gsub('(.*).genes_(.*)','\\2',L1))
profil.set = profil.set %>% mutate(group = gsub('(.*).genes_(.*)','\\1',L1),
                                   feature = gsub('(.*).genes_(.*)','\\2',L1))
```

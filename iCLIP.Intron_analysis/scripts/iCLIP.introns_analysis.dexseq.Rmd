...
```{r eval=FALSE}
enrichment_set = paste0(list.files('../iCLIP.introns_analysis.elav_spliced/iCLIP_enrichment/', pattern = '.*dexseq_.*.tsv$', full.names = TRUE),collapse = ',')
centrality_set = paste0(list.files('../iCLIP.introns_analysis.elav_spliced/iCLIP_centrality/', pattern = '.*dexseq_.*.tsv$', full.names = TRUE),collapse = ',')
profile_set = paste0(list.files('../iCLIP.introns_analysis.elav_spliced/iCLIP_profile/', pattern = '.*dexseq_.*.tsv$', full.names = TRUE),collapse = ',')
```
...
enrich.set = enrich.set %>% mutate(group = gsub('(.*).dexseq_(.*)','\\1', L1),
                                   feature = gsub('(.*).dexseq_(.*)','\\2',L1))
centra.set = centra.set %>% mutate(group = gsub('(.*).dexseq_(.*)','\\1',L1),
                                   feature = gsub('(.*).dexseq_(.*)','\\2',L1))
profil.set = profil.set %>% mutate(group = gsub('(.*).dexseq_(.*)','\\1',L1),
                                   feature = gsub('(.*).dexseq_(.*)','\\2',L1))

```

```{r}
feature.factors = c('upstream','downstream','notflanking','geneset')
...
```{r}
ggplot(profil.set %>% filter(feature %in% c('upstream','downstream')), aes(relative_position)) +
  geom_density(show.legend = FALSE) + 
  geom_histogram(aes(y = ..density.., fill = as.character(subjectHits)), binwidth = 0.05, show.legend = FALSE) + 
  scale_fill_viridis_d() +
...
```{r}
ggplot(profil.set %>% filter(feature %in% c('upstream','downstream')), aes(x = bins)) +
  geom_bar(aes(weight = xl.log_density)) +
  theme_bw() +
  facet_grid(group~feature) +
  theme_bw()
...
```{r message=FALSE, warning=FALSE}
ggplot(dt0 %>% filter(feature %in% c('upstream','downstream')), aes(variable)) + geom_bar(aes(weight=value, fill = variable), show.legend = FALSE) + 
  facet_grid(group~feature) + 
  xlab('') + 
  theme_bw()
```

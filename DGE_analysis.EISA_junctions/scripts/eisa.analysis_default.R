...
# Rscript /data/hilgers/group2/rauer/circCenters/output3/DGE_analysis.EISA_junctions/scripts/eisa.analysis_classic.R 
#  -g featureCounts.classic_paired/featureCounts_genes.tsv 
#  -i featureCounts.classic_paired/featureCounts_introns.tsv 
#  --gtf /data/hilgers/group2/rauer/circCenters/output3/DGE_analysis.EISA_junctions/data/resources/dm6_ensembl96.gtf 
#  -s /data/hilgers/group2/rauer/circCenters/output3/DGE_analysis.EISA_junctions/Samplesheets/DoubleMutant.Mutants.tsv 
#  -o eisa_analysis_classic.paired/eisa_mutant
#
# opt$genes = "../EISA_junc.double_mutants//featureCounts.classic_paired/featureCounts_genes.tsv"
# opt$introns = "../EISA_junc.double_mutants/featureCounts.classic_paired/featureCounts_introns.tsv"
# opt$gtf = "/data/hilgers/group2/rauer/circCenters/output3/DGE_analysis.EISA_junctions/data/resources/dm6_ensembl96.gtf"
# opt$samplesheet = '/data/hilgers/group2/rauer/circCenters/output3/DGE_analysis.EISA_junctions/Samplesheets/DoubleMutant.Mutants.tsv'
...
cntEx_raw <- read_tsv(opt$exon_count, comment = '#', show_col_types = FALSE); 
colnames(cntEx_raw) = basename(colnames(cntEx_raw))
cntEx <- (cntEx_raw %>% column_to_rownames('Geneid')) %>% dplyr::select(-Chr,-Start,-End,-Strand,-Length)
...
cntIn = cntGb - cntEx
gid.keep2 = rownames(cntEx)[apply(cntIn, 1, function(x) all(x >= 0))]; 

cat("Expressed genes: ", length(gid.keep),'\n')
cat("Discarded ", nrow(cntEx) - length(gid.keep2), 'genes due to negative counts\n')
cat("Final gene count: ", length(gid.keep2), '\n')
